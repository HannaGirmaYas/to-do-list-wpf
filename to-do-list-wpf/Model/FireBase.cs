...
namespace to_do_list_wpf.Model
{
    class FBToDo
    {
        ...
        //for checkItems...
        public to_doTask.ChecklistItem AddCheckListItem(string username, string title, to_do_list_wpf.Model.to_doTask.ChecklistItem c)
        {
            SetResponse response = client.Set("ToDoList/Users/" + username + "/TaskList/" + title + "/ChecklistItem/" + c.Text, c);
            return response.ResultAs<to_doTask.ChecklistItem>();
        }
        public List<to_doTask.ChecklistItem> GetCheckListItems(string username, string title)
        {
            FirebaseResponse response = client.Get("ToDoList/Users/" + username + "/TaskList/" + title + "/ChecklistItem");
